...
import { Id, Value, Entities, Statuses, Targets, TEntities } from "Status";

const isNotEmpty = complement(either(isNil, isEmpty));

//   [SELECTED]: {
//     [TABLE]:{{id1: true} ...},
//     [COLUMN]:{{id1: true}, id2: true, ...},
//   }

function reduce<S extends Id, E extends Id, U>(
  targets: any[],
  value: U,
  initValue?: Statuses<S, E, TEntities<E>> | TEntities<E> | Targets<Value>,
  replaceAll?: boolean
): Statuses<S, E, TEntities<E>> | TEntities<E> | Targets<Value> {
  return targets.reduce((acc, id) => {
    if (!acc[id]) {
      acc[id] = value;
    }
    return acc;
  }, initValue || ({} as Statuses<S, E, TEntities<E>> | TEntities<E> | Targets<Value>));
}

class Status<S extends Id, E extends Id> {
  all: Statuses<S, E, TEntities<E>>;
  constructor(
    all: Statuses<S, E, TEntities<E>> = {},
    statusesTypes: S,
    entityTypes: E
    ...
    // Init all with empty Statuses  ie: {[SELECTED]:{}, [FOCUSED]: {}, ...}
    this.all = {}; //reduce(values(Object(statusesTypes)), {}, {});
  }
  ...
  isAlready(key: Id, statusType: S, entityType: E): boolean {
    if (entityType) {
      ...
      return isNotEmpty(inStatus)
        ? keys(inStatus).includes(key as string)
        : false;
    } else {
      const allEntityTypesIds = this.mergeEntityTypes(statusType);
      return isNotEmpty(allEntityTypesIds)
        ? keys(allEntityTypesIds).includes(key)
        : false;
    }
  }
  ...
  hasMultiple(statusType: S, entityType: E): boolean {
    if (entityType) {
      ...
      const allEntityTypesIds = this.mergeEntityTypes(statusType);
      return isNotEmpty(allEntityTypesIds)
        ? keys(allEntityTypesIds).length > 1
        : false;
    }
  }
  ...
  reset(statusType: S, entityType: E): void {
    ...
    if (!entityType) {
      ...
      const entities = this.all[statusType] as TEntities<E>;
      if (!entities) {
        this.all[statusType] = {};
      } else {
  ...
  setTargets(statusType: S, entityType: E, targets: Targets<Value>) {
    this.all = { ...this.all, [statusType]: { [entityType]: targets } };
  }

  mergeValues(statusType: S, entityType: E, targets: Targets<Value>, key: Id) {
    const entities = this.all[statusType];
    if (!entities) {
      return;
    }
    const newEntities: TEntities<E> = {
      ...entities,
  ...
  set(
    ...
    if (replaceAll) {
      ...
      if (statusType && entityType) {
        const entities = { [entityType]: targets } as TEntities<E>;
        if (!entities) {
        ...
        if (targets[key]) {
          if (!entities) {
            ...
            if (!entities[entityType]) {
              const newEntities = { [entityType]: {} } as TEntities<E>;
              this.all[statusType] = {
                ...this.all[statusType],
                ...newEntities
              };
            }
            this.mergeValues(statusType, entityType, targets, key);
          }
        } else {
